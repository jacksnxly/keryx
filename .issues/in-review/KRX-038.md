...
## Problem
Version parsing errors are completely discarded with `.ok()`. If a changelog has malformed version strings, users get no indication that detection failed—leading to confusing duplicate version warnings.
